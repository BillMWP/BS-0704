...
class StockBackorderConfirmation(models.TransientModel):
    ...
    def create_selected_backorder(self):
        ...
        for product in products:
            ...
            move_id = self.env['stock.move'].search([('picking_id','=',pick_id[0].id),('product_id','=',product.id)]).filtered(lambda l:l.product_uom_qty != l.quantity_done)
            demand = 0
            done = 0
            for move in move_id:
                ...
                move_id.write(
                    {'state': 'done',
                     'product_uom_qty': move_id.quantity_done,
                     })
